    ...
    btn.addEventListener("click", (event) => {
        ...
        while (target.parentNode) {
            if (target.dataset.idHolder) {
                userID = target.dataset.idHolder
                break;
            }
        ...
        modalMessageAcDel.innerHTML = "Are you sure you want to delete this user ID: " + userID + " ?";
        okAcDel.addEventListener("click", () => {
            let xhttp = new XMLHttpRequest();
            xhttp.open("DELETE", `users/${userID}`, true);
            xhttp.send();
                ...
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    if (responseText.redirect) {
                        ...
                        if (responseText.status) {
                            modalMessageArDel.innerHTML = "User ID: " + responseText.deletedId._id + " has been deleted.";
